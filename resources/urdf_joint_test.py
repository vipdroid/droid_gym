...
print("ASSET_ROOT Path：", ASSET_ROOT)
...
asset_descriptors = [
    AssetDesc("robots/X02Lite/X02Lite.xml", False)
]
...
print("+-----+-----------------------+------------+-------------+---------+-----------+---------+---------+---------+")
print("| DOF |          Name         |     Type   |  Stiffness  | Damping |  Armature | Limited |  Lower  |  Upper  |")
print("+-----+-----------------------+------------+-------------+---------+-----------+---------+---------+---------+")
for i in range(num_dofs):
    print("|  %2d |%22s |%10s  |   %7.3f   | %7.3f |   %7.3f |    %d    | %7.4f | %7.4f |"
          %(i, dof_names[i], gym.get_dof_type_string(dof_types[i]), stiffnesses[i],
            dampings[i], armatures[i],has_limits[i], lower_limits[i], upper_limits[i]))
print("+-----+-----------------------+------------+-------------+---------+-----------+---------+---------+---------+")
...
# position the camera
cam_pos = gymapi.Vec3(17.2, 0.625, 16)
...
for i in range(num_envs):
    ...
    pose.p = gymapi.Vec3(0.0, 1.2, 0.0)
    pose.r = gymapi.Quat(-0.707107, 0.0, 0.0, 0.707107)

    actor_handle = gym.create_actor(env, asset, pose, "actor", i, 1)
    actor_handles.append(actor_handle)
    gym.set_rigid_body_color(
        env, actor_handle, 0, gymapi.MESH_VISUAL, gymapi.Vec3(1, 0, 0))
